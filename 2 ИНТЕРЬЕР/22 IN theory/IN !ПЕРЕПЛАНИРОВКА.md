---
DATE: 2025-05-13 14:09
CLASS: -20 ИНТЕРЬЕР
TYPE: ПЛАНИРОВКА
SubTYPE: перепланировка
cover: "![[Pasted image 20250513142718.png|200]]"
note: Принципы перпланировки
excalidraw-plugin: parsed
tags:
  - excalidraw
  - in_stage_2_theory
excalidraw-open-md: true
---
## *\*cover*
![[Pasted image 20250513142718.png]]
## *\*table*
```dataview
TABLE
	note as "Примечание",
	tags as "Tags",
	cover as "Cover"
WHERE CLASS = "-" where SubTYPE = "general" where TYPE != "GENERAL"
SORT published
```
- 
---
# [[IN !ПЕРЕПЛАНИРОВКА]]
- [[IN !ПЕРЕПЛАНИРОВКА]]
---
## **ADD**
- **ADD**
---
### заголовок

---
## *МАТЕРИАЛЫ*
- *МАТЕРИАЛЫ*
---
### заголовок

---
## main
- main
---
### **ОСНОВНЫЕ ШАГИ**
- **ОСНОВНЫЕ ШАГИ**
---
- [ ] **ПОДГОТОВКА ИСХОДНИКА** 
	- -> подготовка плана на *л.(1)4.0E -- ФУНКЦИОНАЛЬНОЕ ЗОНИРОВАНИЕ*
	- [ ] Сделать чистый план -> *снести стены, кроме:*
		- [ ] несущих
	- [ ] Нанести контур помещений:
		- [ ] санузел
		- [ ] коридор
		- [ ] кухня
	- [ ] Нанести оборудование:
		- [ ] инженерное оборудование -> *трубы, щиток и т.п.*

- [ ] **ТЗ, СЦЕНАРИИ**
	- [ ] Заполнение ТЗ [excell -> Техническое задание, КП, договор](https://disk.yandex.ru/i/QTMG5VV-RPAiyA)
	- [ ] Определить потребности каждого жильца, питомцев -> *сценарии*
		- [ ] *помещения*
			- [ ] по сезонам
		- [ ] сценарии для питомцев
		- [ ] когда приходят гости
		- [ ] *освещение*
			- [ ] утро
			- [ ] день
			- [ ] вечер
			- [ ] ночь
	- [ ] *СЦЕНАРИИ*
		- [ ] Сценарий *утром*
			- [ ] освещение
			- [ ] хранение вещей
			- [ ] спорт
			- [ ] хобби
			- [ ] отдых
			- [ ] работа
			- [ ] игры
			- [ ] гости
		- [ ] Сценарий *днём*
			- [ ] освещение
			- [ ] хранение вещей
			- [ ] спорт
			- [ ] хобби
			- [ ] отдых
			- [ ] работа
			- [ ] игры
			- [ ] гости
		- [ ] Сценарий *вечером*
			- [ ] освещение
			- [ ] хранение вещей
			- [ ] спорт
			- [ ] хобби
			- [ ] отдых
			- [ ] работа
			- [ ] игры
			- [ ] гости
		- [ ] Сценарий *ночью*
			- [ ] освещение
			- [ ] хранение вещей
			- [ ] спорт
			- [ ] хобби
			- [ ] отдых
			- [ ] работа
			- [ ] игры
			- [ ] гости

- [ ] **ОТРИСОВКА ПЛАНА**
	- [ ] Выбрать один из вариантов:
		- [ ] От руки -> *на листочке* -> А4 или больше формат
		- [ ] *Программа:*
			- [ ] Figma
			- [ ] Photoshop
			- [ ] Procreate
			- [ ] или что больше нравится или где удобнее это будет делать
	- [ ] Перенести план 

- [ ] **ЗОНИРОВАНИЕ**
	- [ ] *КРУПНОЕ ЗОНИРОВАНИЕ*
		- [ ] *Входная зона* 
			- [ ] прихожая
			- [ ] коридор    
		- [ ] *Гигиена* 
			- [ ] cанузел
			- [ ] ванная		    
		- [ ] *Приготовление и приём пищи* 
			- [ ] кухня
			- [ ] столовая
			- [ ] кухня-столовая		    
		- [ ] *Быт родителей*
			- [ ] спальня
			- [ ] рабочее место		    
		- [ ] *Быт детей*
			- [ ] спальня
			- [ ] детская		    
		- [ ] *Общая зона*
			- [ ] общая комната
			- [ ] гостиная
	- [ ] *ПРОРАБАТЫВАЕМ КРУПНЫЕ ЗОНЫ*
		- [ ] **ВХОДНАЯ ЗОНА**
			- [ ] *Входная зона* – прихожая, коридор    
			- [ ] *Гигиена* – санузел, ванная    
			- [ ] *Приготовление и приём пищи* – кухня, столовая, кухня-столовая    
			- [ ] *Быт родителей* – спальня, рабочее место,    
			- [ ] *Быт детей* – спальня, детская    
			- [ ] *Общая зона* – общая комната, гостиная
		- [ ] **ГИГИЕНА**
			- [ ] *Туалет* (унитаз)  
			- [ ] *Умывальник*  
			- [ ] *Ванная, душевая*  
			- [ ] *Хранение* 
			- [ ] *Постирочная прачечная*
		- [ ] **ГОТОВКА и ПРИЁМ ПИЩИ**
			- [ ] *Приготовление еды*  
			- [ ] *Хранение еды и приборов* (кастрюль, сковородок, посуды, бытовой химии)  
			- [ ] *Приём пищи*
		- [ ] **ОБЩАЯ ЗОНА**
			- [ ] *Место отдыха*  
			- [ ] *Детская*  
			- [ ] *Рабочее место* 
			- [ ] *Хобби*  
			- [ ] *Хранение*  
			- [ ] *Телевизионная*  
			- [ ] *Спорт*  
			- [ ] *Приём гостей*
		- [ ] **БЫТ РОДИТЕЛЕЙ** 
			- [ ] *Место для сна  
			- [ ] *Рабочее место  
			- [ ] *Хобби*
			- [ ] *Хранение  
			- [ ] *Спорт
		- [ ] **БЫТ ДЕТЕЙ** 
			- [ ] *Место для сна*
			- [ ] *Рабочее место* 
			- [ ] *Хобби*  
			- [ ] *Хранение*
		- [ ] **ПИТОМЦЫ** 
			- [ ] *Место для сна*
			- [ ] *Еда* 
			- [ ] *Туалет*  
			- [ ] *Игры*
	- [ ] *ИНСОЛЯЦИЯ*
		- [ ] Расположение помещений по инсоляции
		- ![[Pasted image 20250513154019.png|200]]
		- ![[Pasted image 20250513154143.png|200]]

- [ ] **ПОМЕЩЕНИЯ** -> *ПРИХОЖАЯ*
	- Начать планировку с **ПРИХОЖЕЙ** *(она ограничивает другие пространства)*
	- [ ] выход без дверей
	- [ ] ширина -> лучше **1400 мм**
	- [ ] 

---
### **ЗАГОЛОВОК**
- **ЗАГОЛОВОК**
---


---
### **ЗАГОЛОВОК**
- **ЗАГОЛОВОК**
---


---
# Desk - IN !ПЛАНИРОВКА

![[IN !ПЛАНИРОВКА.svg]]
%%
# Excalidraw Data
## Text Elements
BACKLOG ^irN3rVwb

TO DO ^ktx2G2TZ

WIP ^C2Iy3HOt

DONE ^yZwFVIuD

## Drawing
```compressed-json

```
%%